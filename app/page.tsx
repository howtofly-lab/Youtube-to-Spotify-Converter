...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        alert(
          "In order to continue, your YouTube account must have a channel. Please create a channel and return."
        );
      }
    ...
    async function docs() {
      if (session) {
        if (session?.status == "authenticated") {
          const q = query(
            collection(db, "users"),
            where("__name__", "==", session?.data?.user?.id as string)
          );
  ...
  return (
    <div className="overflow-hidden bg-gradient-to-tr from-gray-800 to-black w-screen h-screen font-sans font-medium">
      ...
      {session.status === "authenticated" ? (
        <div>
          <div
            ...
            className="absolute top-8 right-8 w-20 h-20 rounded-full bg-gray-300 overflow-hidden justify-items-center"
          >
            <Image
              src={currentUser["image" as any]}
              alt=""
              width={80}
              height={80}
            />
          ...
          {dropdown && (
            <div className="dropdown-menu absolute top-[115px] right-10 h-12 w-32 rounded-md bg-neutral-200">
              <button
                className="h-12 w-32 rounded-md bg-neutral-200 hover:bg-green-200 text-lg text-slate-800 transition-colors"
                onClick={() => fullLogout()}
              >
                Logout
              </button>
      ...
      {session.status === "authenticated" ? (
        <div className=" absolute top-[12%] left-10  text-neutral-200 text-5xl drop-shadow-lg">
          {" "}
          Hello, {currentUser["name" as any]}!{" "}
        </div>
      ) : (
        <div></div>
      )}
      ...
      {session.status === "authenticated" ? (
        <div
          className={
            "h-[73%] w-3/4 absolute left-10 top-1/4 mt-4 rounded-tr-lg rounded-tl-lg bg-gradient-to-br from-green-700 to-neutral-200 shadow-lg"
          }
        >
          <div id="container" className="flex justify-center">
            <div className="absolute left-10 top-5 text-slate-800 text-xl">
              To get started, choose a playlist from your YouTube Library:
            </div>
            <form>
              {loadingP ? (
                <div className="absolute left-1/4 top-[15%]">
                  ...
                  >
                    <svg
                      aria-hidden="true"
                      className="w-8 h-8 mr-2 text-lg text-gray-200 animate-spin fill-slate-700"
                      viewBox="0 0 100 101"
                      fill="none"
                ...
                <label>
                  <select
                    ...
                    className="z-[1000] accent-green-400 text-slate-800 text-xl w-2/3 h-12 absolute left-[15%] top-[15%]  focus:ring-2 focus:ring-green-600 focus:h-64 shadow-lg rounded-lg border-r-8 border-white pr-4"
                  >
                    <option
                      key="-1"
                      pl-id="-1"
                    ...
                    {playlists.map((pl: any) => (
                      <option
                        className="text-xl hover:bg-green-200 h-10 pb-2 checked:h-10 accent-green-400"
                        key={pl.id}
                        pl-id={pl.id}
                        value={pl.id}
                      >
          ...
          <div className="flex items-center justify-center text-lg">
            <div className="absolute top-[40%]">
              ...
              >
                <svg
                  aria-hidden="true"
                  className="w-16 h-16 mr-2 text-lg text-gray-200 animate-spin fill-slate-700"
                  viewBox="0 0 100 101"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
